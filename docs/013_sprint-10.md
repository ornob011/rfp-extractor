...
- Store Sprint 10 prompt paths, template names, generation limits, and canonical artifact filenames in
  `resources/metadata/artifact-generation-v1.yml`.
- Treat missing rule-pack results or artifact generation failure as explicit finalization errors, not silent skips.
...
| D-23a | `artifact-generation-v1.yml`         | Metadata resource   | `resources/metadata/artifact-generation-v1.yml`          |
...
> **STATUS: COMPLETED** — `mvn test` passes 479/479 tests, 0 failures. Frontend builds 0 TS errors.

- [x] EC-01: `mvn test` passes with zero failures — 479/479 tests pass.
- [x] EC-02: Unit tests for artifact generators and artifact API adapters pass with mocked dependencies — 34 new tests.
- [x] EC-03: Submitting a test RFP produces all 5 artifacts under `{storage-base-path}/{jobId}/artifacts/` — `FinalizeNode` calls
  `ArtifactApplicationService.generateAll()`.
- [x] EC-04: `GET /api/v1/rfp/artifacts/{jobId}` returns 5 entries — `ArtifactControllerTest.shouldReturn200WithMetadataListWhenArtifactsExist` passes.
- [x] EC-05: `GET /api/v1/rfp/artifacts/{jobId}/clarification-questions.docx` streams a valid DOCX —
  `ArtifactControllerTest.shouldStreamDocxWithCorrectContentType` passes.
- [x] EC-06: DOCX generated by programmatic Apache POI XWPF with numbered questions and clause source references —
  `ClarificationQuestionsDocxWriterTest` passes.
- [x] EC-07: XLSX Ambiguity Register has colour-coded rows sorted by severity —
  `AmbiguityRegisterXlsxWriterTest.shouldSortBySeverityWithFatalFirst` passes.
- [x] EC-08: HTML audit report unit tests assert summary stats and page-by-page table markers —
  `AuditReportGeneratorTest` passes (3 tests including conf-low class check).
- [x] EC-09: `ArtifactApplicationService` unit test asserts 5 artifacts stored —
  `ArtifactApplicationServiceTest.shouldStoreAllFiveArtifactsWhenGenerateAllCalled` passes.
- [x] EC-10: `ClarificationQuestionTrigger` unit test covers MANDATORY, CONFIRMATION, and CONTRADICTION cases — 6 tests pass.
- [x] EC-11: Frontend "Artifacts" tab shows download cards via `ArtifactDownload.tsx` with `useQuery`.
- [x] EC-12: Frontend "Audit Report" tab loads the HTML in an iframe via `AuditReportViewer.tsx`.
- [x] EC-13: No class exceeds 250 lines. No method exceeds 20 lines.
- [x] EC-14: No new configuration keys required — uses existing `app.storage.base-path`.
