...
| 11     | `014`  | **Security & RBAC**                            | Public signup/login + token lifecycle (access + rotating refresh), enum-based roles (ANALYST/ADMIN/AUDITOR), per-document ownership, AES-256 encryption at rest, data retention scheduler, prompt injection filter, legacy Bangla rejection | Sprint 10 done | Signup/login/refresh/logout flow works; ANALYST cannot access another user's doc; encryption verified; rejection error for legacy Bangla | Demo role-based access and signup/login flow                           |
