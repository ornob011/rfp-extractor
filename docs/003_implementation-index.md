...
| 8      | `011`  | **Rule Pack Engine & ICT Rules** ✅               | `RulePackRunner` (JMESPath + LLM judgment), `RfpTypeClassifier`, `bd-govt-ict-v1.yaml` (80 rules), rule test suite                                                                                                                                                    | Sprint 8 done  | All 80 ICT rules run; FATAL rules fire on docs missing mandatory fields                                                                  | Show PASS/FAIL rule results colour-coded by severity                   |
| 9      | `012`  | **Additional Rule Packs** ✅                      | Works, consultancy, goods rule packs; admin API for rule pack management                                                                                                                                                                                              | Sprint 8 done  | Works/consultancy/goods rules run on respective doc types; admin-triggered reload verified                                               | Trigger admin reload and verify the refreshed pack catalog             |
...
| 11     | `014`  | **Security & RBAC** ✅                            | Spring Security layer on existing `UserEntity` (Sprint 1): JWT auth, `JpaUserDetailsService`, BCrypt, RBAC (ANALYST/ADMIN/AUDITOR), per-document ownership, AES-256 encryption at rest, prompt injection filter, legacy Bangla rejection | Sprint 10 done | Signup/login/refresh/logout flow works; ANALYST cannot access another user's doc; encryption verified; rejection error for legacy Bangla | Demo role-based access and signup/login flow                           |
...
| [012_sprint-9.md](012_sprint-9.md)   | Sprint 9  | Additional Rule Packs & Admin Reload       |
